...
# pygmentize -S gruvbox-dark -f html -a .terror > pygments-gruvbox-dark.css
# where -S reference the style, and  -a .terror is the class name that will be used in the HTML file to apply the style
...
#
# in case the interest is not to use the colors defined in the CSS file, formatter.noclasses = True, have to be set
#
# formatter = HtmlFormatter(style='gruvbox-light', cssclass = 'gruvebox-dark-style', linenos=True)
# formatter.noclasses = True
#
# in this case style='gruvbox-light', will be provided as an inline CSS styles, in the html generated by pygments_highlight()
#
# For the Math formulas rendering I attempted to use MathJax en the server side but I'm having issues implementing it
# using the texmath_plugin from mdit_py_plugins
# so I decided to for now render them on the client side
...
def pygments_highlight(code, lang):
    ...
    formatter.noclasses = True   # Decided to not use the CSS file for now, I think it only have sense for a personalized theme
    # return highlight(code, PythonLexer(), formatter) # this works! but will keep duing it by hand
